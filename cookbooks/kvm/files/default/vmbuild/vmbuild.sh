...
lvcreate -n kvm_${NAME} -L 18G VolGroupKVM
vmbuilder kvm ubuntu --suite=intrepid --flavour=virtual --arch=amd64 --hostname=${NAME} --mem=${MEM} --cpus=${CPU}\
    ...
    --bridge=br0 --ip=${IP} --mask=255.255.252.0 --net=192.168.0.0 --bcast=192.168.3.255 --gw=192.168.1.1 --dns=192.168.1.157 \
    ...
    --templates=/usr/local/vmbuild/templates \
    --copy=/usr/local/vmbuild/files/manifest.txt \
    --execscript=/usr/local/vmbuild/scripts/postinstall.sh \
